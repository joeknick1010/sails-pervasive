...
const dbQuery = (sql, data, callback) => {
  ...
  const execQuery = (sql, data, callback) => {
    db.query(sql, data, (err, res) => {
      if (err) {
        if (attempts === 0 &&
          err.message.indexOf('Your connection to the server is no longer working') >= 0) {
          attempts += 1;
          dbConnected = false;
          log.warn('Lost connection with the server. Reconnecting');
          db.close(() => {
  ...
  _queryWithConnect = function _queryWithConnect(sql, data, callback) {
    if (log) {
